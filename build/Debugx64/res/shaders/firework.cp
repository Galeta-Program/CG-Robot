...
    // Only process particles in the specified range

    if(index < u_FromIndex || index > u_FromIndex + u_Size - 1)

    {

        return; // Don't modify particles outside our range
...
        float particleSeed = float(index);

        float baseInitialLifetime = 6.0;    
...
        particle.size = u_EmitterParticleSize + hash11(particleSeed * 1.41) * 5.0; // 8 ~ 13

        particle.initialSize = particle.size; // 8 ~ 13

        

        vec3 noiseSpawnDir = hash31(particleSeed * 4.56);

        vec3 initialDir = normalize(noiseSpawnDir);

        

        float newV = u_Velocity + hash11(particleSeed * 7.89) * 3.0;

        particle.velocity = vec4(initialDir, newV);



        particle.color = vec4(hash31(particleSeed * 5.67), 1.0); 

        particle.acceleration = vec4(0.0, 0.0, 0.0, 0.0);

        

        float explosionTime = hash11(particleSeed * 2.33) * 0.5; 

        particle.pos += initialDir * newV * explosionTime; 

        particle.lifetime -= explosionTime;
...
        float f_invLifeRatio = invLifeRatio; 
...
        particle.color.a = max(0.0f, particle.color.a); 
